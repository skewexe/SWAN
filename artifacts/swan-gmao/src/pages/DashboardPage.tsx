...
type DashboardWidgetId = "kpis" | "monthly" | "category" | "status" | "priority" | "availability" | "agenda" | "alerts" | "activity";
...
const DEFAULT_WIDGETS: DashboardWidget[] = [
  { id: "kpis", title: "KPI", subtitle: "Cartes clés", kind: "kpis", span: "full" },
  { id: "monthly", title: "Interventions par mois", subtitle: "Barres", kind: "bar", span: "half" },
  { id: "category", title: "Par catégorie", subtitle: "Répartition", kind: "pie", span: "third" },
  { id: "status", title: "Statut OT", subtitle: "Pilotage", kind: "status", span: "half" },
  { id: "priority", title: "Priorité OT", subtitle: "Criticité", kind: "priority", span: "half" },
  { id: "availability", title: "Disponibilité", subtitle: "Tendance", kind: "trend", span: "full" },
  { id: "agenda", title: "Agenda", subtitle: "Semaine", kind: "agenda", span: "half" },
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setWidgets(JSON.parse(raw) as DashboardWidget[]);
    } catch {}
  }, []);
        ...
        {widgets.map((widget, idx) => (
          <motion.div
            ...
            onDrop={() => {
              ...
              const from = widgets.findIndex(w => w.id === draggedId);
              const to = widgets.findIndex(w => w.id === widget.id);
              moveWidget(from, to);
              setDraggedId(null);
            }}
          ...
          >
              ...
              <div className="bg-card border border-border/60 rounded-2xl p-6 min-h-[360px]">
                ...
                {chartLoading ? <Skeleton className="h-52 mt-4" /> : filteredChartData ? (
                    ...
                    <BarChart data={filteredChartData.maintenanceByMonth} barCategoryGap={12} barGap={4} onClick={(data) => { if (data?.activeLabel) setFilter("month", data.activeLabel); }}>
                      ...
                      <Legend formatter={(value) => <span style={{ color: "#94A3B8", fontSize: 11 }}>{value}</span>} iconType="circle" iconSize={8} />
                      <Bar dataKey="corrective" name="Corrective" radius={[4, 4, 0, 0]} maxBarSize={22} cursor="pointer">
                        {filteredChartData.maintenanceByMonth.map((entry: any, idx: number) => <Cell key={idx} fill={entry.selected ? "#EF4444" : "#EF444480"} stroke={entry.selected ? "#EF4444" : "none"} strokeWidth={entry.selected ? 2 : 0} />)}
                      </Bar>
                      <Bar dataKey="preventive" name="Préventive" radius={[4, 4, 0, 0]} maxBarSize={22} cursor="pointer">
                        {filteredChartData.maintenanceByMonth.map((entry: any, idx: number) => <Cell key={idx} fill={entry.selected ? "#22C55E" : "#22C55E80"} stroke={entry.selected ? "#22C55E" : "none"} strokeWidth={entry.selected ? 2 : 0} />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                ) : null}
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="w-full space-y-1.5">
                      {filteredChartData.assetsByCategory.map((item: any, idx: number) => (
                        <button key={item.category} onClick={() => setFilter("category", item.category)} className={`w-full flex items-center justify-between text-xs px-2 py-1 rounded-lg transition-colors ${filters.category === item.category ? "bg-primary/10 text-foreground" : "hover:bg-muted/50 text-muted-foreground hover:text-foreground"}`}>
                          <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full" style={{ background: CHART_COLORS[idx % CHART_COLORS.length] }} /><span>{item.category}</span></div>
            ...
            {widget.kind === "status" && (
              <div className="bg-card border border-border/60 rounded-2xl p-5">
                ...
                {wosLoading ? <Skeleton className="h-24" /> : <div className="space-y-2">{woStatusBreakdown.map(({ status, count, pct }) => { const info = WO_STATUS_MAP[status]; const color = STATUS_COLORS[status] || "#94A3B8"; const isActive = filters.status === status; return <button key={status} onClick={() => setFilter("status", status)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all text-left ${isActive ? "bg-primary/5 ring-1 ring-primary/30" : "hover:bg-muted/40"}`}><div className="h-2 w-2 rounded-full shrink-0" style={{ background: color }} /><span className="text-xs font-medium text-foreground flex-1">{info?.label || status}</span><div className="flex items-center gap-3 shrink-0"><div className="w-28 h-1.5 rounded-full bg-muted overflow-hidden"><motion.div className="h-full rounded-full" style={{ background: color }} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.6, delay: 0.1 }} /></div><span className="text-xs font-semibold text-foreground w-8 text-right tabular-nums">{count}</span></div></button>; })}</div>}
              </div>
            )}
            {widget.kind === "priority" && (
              <div className="bg-card border border-border/60 rounded-2xl p-5">
                ...
                {wosLoading ? <Skeleton className="h-24" /> : <div className="space-y-2">{woPriorityBreakdown.map(({ priority, count, pct }) => { const color = PRIORITY_DOT[priority] || "#94A3B8"; const labels: Record<string, string> = { critical: "Critique", high: "Élevée", medium: "Moyenne", low: "Faible" }; const isActive = filters.priority === priority; return <button key={priority} onClick={() => setFilter("priority", priority)} className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl transition-all text-left ${isActive ? "bg-primary/5 ring-1 ring-primary/30" : "hover:bg-muted/40"}`}><div className="h-2 w-2 rounded-full shrink-0" style={{ background: color }} /><span className="text-xs font-medium text-foreground flex-1">{labels[priority] || priority}</span><div className="flex items-center gap-3 shrink-0"><div className="w-28 h-1.5 rounded-full bg-muted overflow-hidden"><motion.div className="h-full rounded-full" style={{ background: color }} initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.6, delay: 0.1 }} /></div><span className="text-xs font-semibold text-foreground w-8 text-right tabular-nums">{count}</span></div></button>; })}</div>}
              </div>
            )}
            {widget.kind === "trend" && (
            ...
            {widget.kind === "agenda" && (
              <div className="bg-card border border-border/60 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-1"><div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-primary" strokeWidth={1.5} /><h3 className="text-sm font-semibold text-foreground">Agenda de la semaine</h3></div><Link href="/calendar"><button className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1 transition-colors">Voir tout <ChevronRight className="h-3 w-3" /></button></Link></div>
                <p className="text-xs text-muted-foreground mb-5">Interventions planifiées dans les 7 prochains jours{activeFilterCount > 0 && <span className="text-primary ml-1">(filtrées)</span>}</p>
                {wosLoading ? <div className="space-y-2">{[1,2,3].map(i => <Skeleton key={i} className="h-14 rounded-xl" />)}</div> : upcomingWOs.length > 0 ? <div className="space-y-2">{upcomingWOs.map((wo, idx) => { const statusInfo = WO_STATUS_MAP[wo.status] || WO_STATUS_MAP.open; const dotColor = PRIORITY_DOT[wo.priority] || "#94A3B8"; const date = new Date(wo.scheduledDate!); const isToday = date.toDateString() === today.toDateString(); return <motion.div key={wo.id} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04 }} className="flex items-center gap-4 bg-background/50 border border-border/40 rounded-xl px-4 py-3 hover:border-primary/30 transition-colors"><div className="h-2 w-2 rounded-full shrink-0" style={{ background: dotColor }} /><div className="flex-1 min-w-0"><div className="text-sm font-medium text-foreground truncate">{wo.title}</div><div className="text-xs text-muted-foreground mt-0.5">{wo.assetName || "—"} · {wo.technicianName || "—"}</div></div><div className="flex items-center gap-2 shrink-0"><span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${statusInfo.bg}`}>{statusInfo.label}</span><span className={`text-xs font-medium tabular-nums ${isToday ? "text-primary font-semibold" : "text-muted-foreground"}`}>{isToday ? "Aujourd'hui" : date.toLocaleDateString("fr-DZ", { day: "2-digit", month: "short" })}</span></div></motion.div>; })}</div> : <div className="text-center py-8 text-sm text-muted-foreground border border-border/30 rounded-xl bg-background/30"><CalendarDays className="h-6 w-6 mx-auto mb-2 text-muted-foreground/30" strokeWidth={1} />Aucune intervention planifiée cette semaine</div>}
              </div>
            )}
            {widget.kind === "alerts" && (
              <div className="bg-card border border-border/60 rounded-2xl p-6">
                <div className="flex items-center gap-2 mb-1"><AlertTriangle className="h-4 w-4 text-yellow-400" strokeWidth={1.5} /><h3 className="text-sm font-semibold text-foreground">Plans en retard</h3></div>
                <p className="text-xs text-muted-foreground mb-5">Maintenances préventives dépassées</p>
                {overduePlans.length > 0 ? <div className="space-y-2">{overduePlans.slice(0, 5).map((plan, idx) => <motion.div key={plan.id} initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04 }} className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl px-4 py-3 hover:border-yellow-500/40 transition-colors"><div className="text-sm font-medium text-foreground leading-snug truncate">{plan.name}</div><div className="text-xs text-muted-foreground mt-0.5">Prévu: <span className="text-yellow-400 font-medium">{plan.nextDue ? new Date(plan.nextDue).toLocaleDateString("fr-DZ", { day: "2-digit", month: "short" }) : "—"}</span></div></motion.div>)}</div> : <div className="text-center py-8 text-sm text-muted-foreground border border-border/30 rounded-xl bg-background/30"><CalendarCheck className="h-6 w-6 mx-auto mb-2 text-green-400/50" strokeWidth={1} /><span className="text-green-400 font-medium">Tout est à jour</span></div>}
              </div>
            )}
            {widget.kind === "activity" && (
              <div className="bg-card border border-border/60 rounded-2xl p-6">
                <div className="mb-5"><p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-primary mb-1">CMMS premium 2026</p><h3 className="text-sm font-semibold text-foreground">Suite maintenance complète</h3><p className="text-xs text-muted-foreground mt-0.5">Gestion technique, gammes opératoires, OT, stocks, planning, KPI, coût ABC et historique.</p></div>
                {activityLoading ? <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-14" />)}</div> : activity && activity.length > 0 ? <div className="space-y-0">{activity.map((item, idx) => <motion.div key={item.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.04 }} className="flex items-start gap-4 py-3 border-b border-border/30 last:border-0 hover:bg-muted/20 px-2 rounded-lg -mx-2 transition-colors"><div className="mt-2 h-2 w-2 rounded-full shrink-0" style={{ background: SEVERITY_COLORS[item.severity] || "#94A3B8" }} /><div className="flex-1 min-w-0"><p className="text-sm text-foreground leading-snug">{item.message}</p><p className="text-xs text-muted-foreground mt-1">{new Date(item.timestamp).toLocaleString("fr-DZ", { dateStyle: "short", timeStyle: "short" })}</p></div><ActivitySeverityBadge severity={item.severity} /></motion.div>)}</div> : <p className="text-sm text-muted-foreground text-center py-8">Aucune activité récente</p>}
              </div>
            )}
          </motion.div>
        ))}
